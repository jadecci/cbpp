...
usage() { echo "
Usage: $0 -d input_dir -y psych_file -v conf_file -m famID_file -i <parc_ind> -r <method> -p <preproc> -c <corr> -t <null_test> -s <fix_seed> -l <sub_list> -o <output_dir>

This script is a wrapper to run parcel-wise CBPP using combined connectivity data from HCP.

Note that all parcels in the atlas will be looped through by default. To only run parcel-wise CBPP for a specific parcel, or for better parallelisation, use the -i option to specify one parcel to run at a time.
...
OPTIONAL ARGUMENTS:
  ...
  -l <sub_list>   absolute path to the subject-list file, where each line of the file contains the subject
                  ...
                  [ default: $ROOT_DIR/bin/sublist/HCP_MNI_\$preproc_allRun_sub.csv ]
...
OUTPUTS:
  ...
  For example: pwCBPP_SVR_standard_fix_wmcsf_AICHA_Pearson.mat
...
# Assign arguments
while getopts "p:c:d:y:v:m:i:r:f:t:s:l:o:h" opt; do
  case $opt in
    p) preproc=${OPTARG} ;;
    c) corr=${OPTARG} ;;
    d) input_dir=${OPTARG} ;;
