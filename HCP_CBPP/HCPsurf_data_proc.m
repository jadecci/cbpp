...
output = fullfile(out_dir, ['HCP_' options.preproc '_' options.atlas '_' options.fc_method '.mat']);
if isfile(output)
...
for sub_ind = 1:length(subjects)
    ...
    for i = 1:length(run)
        ...
        otherwise
            disp('Invalid preprocessing option'); return
        end
...
function global_signal_withDiff(input)
...
function parcellate_Schaefer_fslr(input, level)
