...
$SCRIPT_DIR/extract_csv_by_colHeader.sh $row_temp $SCRIPT_DIR/HCP_psychometric_list.csv $psych_temp
...
# convert extracted data to .mat format
out_prefix=HCP_${space}_${preproc}_sub${sub_start}to${sub_stop}
if [ $unit_test -eq 1 ]; then out_prefix=unit_test_${space}; fi
matlab -nodesktop -nosplash -r "addpath('$SCRIPT_DIR'); \
                                convert_csv_to_mat_HCP('$psych_temp', '$conf_unres_temp', '$conf_res_temp', '$out_dir', '$out_prefix'); \
                                rmpath('$SCRIPT_DIR'); \
                                exit"
...
usage() { echo "
Usage: $0 -i <unres_file> -j <res_file> -s <space> -p <preproc> -a <sub_start> -b <sub_stop> -u <unit_test> -o <out_dir>
...
OUTPUTS:
  ...
  For example: HCP_surf_fix_sub1to923_y.mat
               HCP_surf_fix_sub1to923_conf.mat
...
# Assign arguments
while getopts "i:j:s:p:a:b:u:o:h" opt; do
  case $opt in
    i) unres_file=${OPTARG} ;;
    j) res_file=${OPTARG} ;;
    s) space=${OPTARG} ;;
